...
!define PRODUCT_VERSION         "18.1.5.6"
...
Section "MainSection" SEC01
  ...
  ; packaging machine, hence the exclusion.
  SetOutPath "$INSTDIR"
  File /r /x "Settings.xml" "${SOURCE_PATH}\Settings"
...
Section Uninstall
  ...
  ; The Settings subfolders are installer-owned (cfg + languages): remove them
  ; recursively so newly added vendors/files (eg. LISUAN) never leave leftovers.
  ; The Settings root itself is NOT removed recursively here - the user's runtime
  ; Settings.xml lives there and is only deleted via the checkbox below.
  RMDir /r "$INSTDIR\Settings\AMD"
  RMDir /r "$INSTDIR\Settings\INTEL"
  ...
  ; Only removed when empty: preserves the user's Settings.xml unless the
  ; "Remove settings" checkbox below was checked.
