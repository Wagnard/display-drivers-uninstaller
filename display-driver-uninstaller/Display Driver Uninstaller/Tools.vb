	...
	Public Module Tools
		...
		Public Function IsNet48OrNewer() As Boolean
            Using ndpKey As RegistryKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).OpenSubKey("SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full\")
                ' Key (or the Release value) can be missing if .NET 4.x is not installed —
                ' guard against a NullReferenceException at startup.
                If ndpKey Is Nothing Then Return False

                Dim releaseObj As Object = ndpKey.GetValue("Release")
                If releaseObj Is Nothing Then Return False

                Dim releaseKey As Integer = Convert.ToInt32(releaseObj)
        ...
        Public Function PreferredUILanguages() As String
            Try
                Using regkey As RegistryKey = MyRegistry.OpenSubKey(Registry.CurrentUser, "Control Panel\Desktop", False)
                    ...
                    Else
                        ...
                        Using regkey2 As RegistryKey = MyRegistry.OpenSubKey(Registry.CurrentUser, "Control Panel\Desktop\MuiCached", False)
                            If regkey2 IsNot Nothing Then
                                Dim wantedValue As String() = CType(regkey2.GetValue("MachinePreferredUILanguages"), String())
        ...
        ''' <summary>Folder name only, separator kept ("C:\Users\Bob\nvosc" -> "\nvosc").
        ''' Matching on the full path would catch the user name : "ace" hits every folder of a user named Grace.</summary>
        ...
        Public Sub FixBrokenPathIfNeeded()
            ...
            Using key As RegistryKey = MyRegistry.OpenSubKey(Registry.LocalMachine, "SOFTWARE\Microsoft\Windows NT\CurrentVersion", False)
        ...
        ''' <summary>Alias for MessageBox.Show(message) with defaults: only 'OK' button + 'Information' image</summary>
        Public Function MsgBox(ByVal message As String, Optional ByVal buttons As MessageBoxButton = MessageBoxButton.OK, Optional ByVal image As MessageBoxImage = MessageBoxImage.Information) As MessageBoxResult
            Return System.Windows.MessageBox.Show(message, Application.Settings.AppName, buttons, image)
        End Function

        ''' <summary>Alias for MessageBox.Show(message, title) with defaults: only 'OK' button + 'Information' image</summary>
