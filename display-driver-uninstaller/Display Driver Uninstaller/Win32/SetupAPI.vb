...
#Region "Enums"
		...
		Private Enum CONFIGFLAGS As UInteger
            DISABLED = &H1UI    ' CONFIGFLAG_DISABLED
            REMOVED = &H2UI    ' CONFIGFLAG_REMOVED
            MANUAL_INSTALL = &H4UI    ' CONFIGFLAG_MANUAL_INSTALL
            IGNORE_BOOT_LC = &H8UI    ' CONFIGFLAG_IGNORE_BOOT_LC
            NET_BOOT = &H10UI   ' CONFIGFLAG_NET_BOOT       (was &H16 — decimal 16 mistakenly written as hex)
            REINSTALL = &H20UI   ' CONFIGFLAG_REINSTALL      (was &H32 — decimal 32 mistakenly written as hex)
            FAILEDINSTALL = &H40UI   ' CONFIGFLAG_FAILEDINSTALL  (was &H64 — decimal 64 mistakenly written as hex)
            CANTSTOPACHILD = &H80UI   ' CONFIGFLAG_CANTSTOPACHILD (was &H128 — decimal 128 mistakenly written as hex)
            OKREMOVEROM = &H100UI  ' CONFIGFLAG_OKREMOVEROM    (was &H256 — decimal 256 mistakenly written as hex)
            NOREMOVEEXIT = &H200UI  ' CONFIGFLAG_NOREMOVEEXIT   (was &H512 — decimal 512 mistakenly written as hex)
        End Enum
...
#Region "P/Invoke CfgMgr32"
        ...
        ' CfgMgr32 functions return CONFIGRET directly and do NOT call SetLastError.
        ' SetLastError:=False prevents the P/Invoke marshaller from clobbering a real
        ' Win32 error from a preceding call with whatever happens to be in TLS at the time.
...
#Region "P/Invoke SetupAPI"
  ...
  <[Out]()> ByVal DeviceInfoData As IntPtr) As <MarshalAs(UnmanagedType.Bool)> Boolean
   ...
   <[In]()> ByVal Reserved As IntPtr) As <MarshalAs(UnmanagedType.Bool)> Boolean
        ...
        ' DriverInfoData is passed as IntPtr (ptrDrvInfoData.Ptr) so the caller can supply
        ' the architecture-correct struct (SP_DRVINFO_DATA_X64 or X86) via StructPtr,
        ' exactly like SetupDiEnumDriverInfo and SetupDiGetDriverInfoDetail.
    ...
    ByVal DriverInfoData As IntPtr) As Boolean
        End Function

        <DllImport("setupapi.dll", SetLastError:=True, CharSet:=CharSet.Auto)>
        Private Shared Function SetupDiGetDeviceProperty(
        ByVal DeviceInfoSet As SafeDeviceHandle,
        ByRef DeviceInfoData As SP_DEVINFO_DATA,
        ByRef PropertyKey As DEVPROPKEY,
        ByRef PropertyType As Integer,
        ByVal PropertyBuffer As IntPtr,
...
#Region "Functions"
            ...
            Try
                ...
                Using infoSet As SafeDeviceHandle = SetupDiGetClassDevs(IntPtr.Zero, Nothing, IntPtr.Zero, DIGCF.ALLCLASSES)
                    ...
                    Try
                        ...
                        If Not enable Then
                            logStatus.Message = "Disabling device..."
                        Else
                            logStatus.Message = "Enabling device..."
                        End If
                    ...
                    Try
                        ...
                        If device.ExtendedInfs IsNot Nothing AndAlso device.ExtendedInfs.Length > 0 Then
                            For Each extendedInf As String In device.ExtendedInfs
                                If String.IsNullOrWhiteSpace(extendedInf) Then
                                    Continue For
                                End If
        ...
        Public Shared Sub RemoveInf(ByVal oem As Inf, ByVal force As Boolean, Optional ByVal OnlySetupUninstallOEMInf As Boolean = False)
            ...
            Try
                ...
                If Not oem.FileExists Then
                    ' This should not happen under normal operation — the caller is expected to
                    ' verify existence before calling RemoveInf.  Log a warning so it shows up
                    ' in the log without being a hard error.
                    Application.Log.AddWarningMessage(String.Format(
                        "RemoveInf skipped — INF file is absent or has no filename: {0}",
                        If(String.IsNullOrWhiteSpace(oem.FileName), "<empty>", oem.FileName)))
                    Return
                End If
            ...
            Finally
                ' Only add the log entry if it was actually populated (i.e. we got past the
                ' early-return guards at the top of the method).
                If Not String.IsNullOrWhiteSpace(logInfs.Message) Then
                    Application.Log.Add(logInfs)
                End If
            End Try
        End Sub

        Private Shared Function GetStringListProperty(
        deviceInfoSet As SafeDeviceHandle,
        deviceInfoData As SP_DEVINFO_DATA,
        ByRef propertyKey As DEVPROPKEY) As List(Of String)

            Dim result As New List(Of String)
            Dim propertyType As Integer = 0
            Dim requiredSize As Integer = 0

            ' First call to get required buffer size
            SetupDiGetDeviceProperty(deviceInfoSet, deviceInfoData, propertyKey,
                               propertyType, IntPtr.Zero, 0, requiredSize, 0)

            If requiredSize = 0 Then Return result

            ' Allocate buffer
            Dim propertyBuffer As IntPtr = Marshal.AllocHGlobal(requiredSize)

            Try
                If SetupDiGetDeviceProperty(deviceInfoSet, deviceInfoData, propertyKey,
                                      propertyType, propertyBuffer, requiredSize, requiredSize, 0) Then

                    ' For STRING_LIST, strings are null-terminated with an extra null at the end
                    Dim offset As Integer = 0
                    While offset < requiredSize - 2  ' -2 to account for final null terminator
                        Dim str As String = Marshal.PtrToStringAuto(IntPtr.Add(propertyBuffer, offset))
                        If String.IsNullOrEmpty(str) Then Exit While

                        result.Add(str)
                        offset += (str.Length + 1) * 2  ' +1 for null terminator, *2 for Unicode
                    End While
                End If
        ...
        Private Shared Sub GetDriverDetails(ByVal infoSet As SafeDeviceHandle, ByVal ptrDevInfo As IntPtr, ByVal device As Device)
            ...
            ' Property key for Extended Configuration IDs
            Dim DEVPKEY_Device_ExtendedConfigurationIds_Guid As New Guid("540b947e-8b40-45bc-a8a2-6a0b894cbda2")
            Const DEVPKEY_Device_ExtendedConfigurationIds_Id As Integer = 15
            Dim extendedInfResult As New List(Of String)
            ...
            Try
                ...
                ' Set up property key
                Dim propertyKey As New DEVPROPKEY()
                propertyKey.fmtid = DEVPKEY_Device_ExtendedConfigurationIds_Guid
                propertyKey.pid = DEVPKEY_Device_ExtendedConfigurationIds_Id

                ' Marshal IntPtr to SP_DEVINFO_DATA
                Dim deviceInfoData As SP_DEVINFO_DATA = Marshal.PtrToStructure(Of SP_DEVINFO_DATA)(ptrDevInfo)

                extendedInfResult.AddRange(GetStringListProperty(infoSet, deviceInfoData, propertyKey))

                device.ExtendedInfs = extendedInfResult.ToArray()
		...
		Private Shared Sub GetChild(ByVal device As Device)
			Try
				...
				result = CM_Get_Child(devInstChild, device.devInst, 0UI)
				...
				If siblingDevices.Count > 0 Then
					device.ChildDevices = siblingDevices.ToArray()
				Else : device.ChildDevices = Nothing
				End If
...
#Region "Classes"
		...
		Public Class Device
			...
			Private _childDevices() As Device
			...
			Public Property ChildDevices As Device()
				Get
			...
			Public Property ExtendedInfs As String()
				Get
					Return _extendedInfs
				End Get
				Friend Set(value As String())
					_extendedInfs = value
				End Set
